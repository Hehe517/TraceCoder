...
class Trainer:
    def __init__(
        ...
        self.gradient_scaler = torch.amp.GradScaler(enabled=self.use_amp)
        specified_path = Path("/path/to/saved_files")
        self.experiment_path = Path(tempfile.mkdtemp(dir=specified_path))
    ...
    def calculate_metrics(
        ...
        if self.threshold_tuning and split_name in ["val", "test"]:
            best_f1_micro, best_f1_macro, best_db = f1_score_db_tuning(logits, targets)


            results_dict[split_name]["all"] |= {
                "f1_micro_tuned": best_f1_micro,
                "f1_macro_tuned": best_f1_macro,
            }

            if evaluating_best_model:

                pprint(f"Best threshold: {best_db}")
                pprint(f"Best F1 micro: {best_f1_micro}")
                pprint(f"Best F1 macro: {best_f1_macro}")
    ...
    def load_checkpoint(self, file_name: str) -> None:
        checkpoint = torch.load(self.experiment_path / file_name, weights_only=True)
